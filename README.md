...
트패픽이 많은 부분은 apache/nginx 의 c/c++ module, erlang, 같은 언어로 개발한다.   
golang 으로 개발을 하는 곳도 많다 러닝커브는 낮고 자바 보다 성능이 좋기 때문이다.   

뛰어난 성능과 안정적으로 서버를 구현할 수 있는 언어는 무었일까?   
...
- 데이터 품질과 정확성 포맷
오류나 불완전한 데이터는 사용자에게 혼란을 줄 수 있다
...
### 사전 준비
- Rust/Cargo, Go, Homebrew 가 설치되어 있어야 합니다.
- macOS에서 빌드 스크립트가 `protoc` 를 필요로 하므로 다음 명령으로 설치합니다.
```
brew install protobuf pkg-config proj
```
- 원본 데이터가 없으면 서버 실행 시 검색 데이터 로딩 단계에서 실패할 수 있습니다.
...
cargo run
```
> `cargo run` 이 `Could not find protoc` 로 실패하면 `brew install protobuf` 를 먼저 실행하세요.
> `failed to load search` 또는 `MissingDbf` 가 나오면 원본 데이터 해제와 변환 단계가 끝났는지 확인하세요.
